...
<section class="indicacoes">
    <div class="container">
        ...
        <?php if ($result->num_rows > 0): ?>
            ...
            <button class="btn btn-link mt-4" type="button" data-bs-toggle="collapse" data-bs-target="#todosVotos">
                Mostrar votos
            </button>

            <div class="collapse" id="todosVotos">
                <div class="card card-body">
                    <?php
                    $sqlDetalhe = "SELECT i.titulo, l.nome 
                                   FROM votos v
                    ...
                    while ($row = $resDetalhe->fetch_assoc()) {
                        $votosDetalhe[$row['titulo']][] = $row['nome'];
                    }
                    ?>
                    <?php foreach ($votosDetalhe as $filme => $usuarios): ?>
                        <p><strong><?= htmlspecialchars($filme) ?>:</strong> <?= implode(", ", $usuarios) ?></p>
                    <?php endforeach; ?>
                </div>
            </div>
